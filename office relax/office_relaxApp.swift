...
class AppDelegate: NSObject, UIApplicationDelegate {
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey : Any]? = nil) -> Bool {
        // 仅做首帧渲染必需的最小初始化，保证先出现界面再处理网络/分析
        FontManager.registerFonts()
        print("===== App启动 =====")
        
        // 其余初始化延后执行，避免网络异常（如 analytics/Firebase 连不上）导致白屏
    ...
    /// 延后执行的启动逻辑（首帧显示后再执行，避免阻塞或网络失败导致白屏）
    private func performDeferredLaunchSetup() {
        print("Bundle Identifier: \(Bundle.main.bundleIdentifier ?? "Unknown")")
        
        // 先完成不依赖 Firebase 的初始化，保证界面可交互
        ...
        // Firebase 必须在主线程配置：Dynamic Links 会创建 WebView，在后台线程会崩溃。
        // 延迟 1 秒再配置，避免阻塞首帧；若网络异常，最多卡一下，不导致白屏。
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
            ...
            for urlString in testURLs {
                self?.testNetworkConnection(urlString: urlString)
            }
        }
    }
